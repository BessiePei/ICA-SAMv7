...
class TestingDataset(Dataset):
    ...
    def __init__(self, data_path, image_size=256, mode='test', requires_name=True, point_num=1, return_ori_mask=True, prompt_path=None, resize_type="nearest", rough_prompt=False):
        """
        ...
        self.rough_prompt = rough_prompt
    ...
    def __getitem__(self, index):
        ...
        image_input = {}
        try:
            image_np = cv2.imread(self.image_paths[index])
        except:
        ...
        transforms = train_transforms(self.image_size, h, w, self.resize_type)
        try:
            augments = transforms(image=image_np, mask=ori_np_mask)
        ...
        if self.rough_prompt:
            pclass = mask_path.split('.')[0][-1]
            rough_mask_path = mask_path.replace('annotations/val2017', 'pixel_prompt/pre_{}'.format(pclass)).replace('.', '-mask.')
            rough_np_mask = cv2.imread(rough_mask_path, 0)
            if rough_np_mask.max() == 255:
                rough_np_mask = rough_np_mask / 255
            rough_mask = torch.tensor(rough_np_mask).unsqueeze(0)
            augments = transforms(image=image_np, mask=rough_np_mask)
            rough_mask = augments['mask'].to(torch.int64)
            boxes = get_boxes_from_mask(rough_mask, self.label_paths[index])
            point_coords, point_labels = init_point_sampling(rough_mask, self.point_num)
            ...
            boxes = get_boxes_from_mask(mask, self.label_paths[index])
...
class TrainingDataset(Dataset):
    def __init__(self, data_dir, image_size=256, mode='train', requires_name=True, point_num=1, mask_num=5, resize_type="nearest"):
        """
    ...
    def __getitem__(self, index):
        ...
        for m in mask_path:
            ...
            boxes = get_boxes_from_mask(mask_tensor, self.label_paths[index])
