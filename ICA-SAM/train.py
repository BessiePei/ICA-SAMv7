...
def print_trainable_parameters(model):
  ...
  for _,param in model.named_parameters():
    ...
    if param.requires_grad:
      trainable_prams += param.numel()
  print(
      f"trainable params: {trainable_prams} || all params: {all_param} || tainable%: {100 * trainable_prams / all_param}"
  )
...
def train_one_epoch(args, model, optimizer, train_loader, epoch, criterion):
    ...
    for batch, batched_input in enumerate(train_loader):
        ...
        if random.random() > 0.5:
            ...
            flag = "point"
        if args.encoder_adapter:      # here change by me
...
def main(args):
    ...
    train_dataset = TrainingDataset(args.data_path, image_size=args.image_size, mode='train', point_num=1, mask_num=args.mask_num, requires_name=False, resize_type = args.resize_type)
    ...
    for epoch in range(0, args.epochs):
        ...
        save_path = os.path.join(args.work_dir, "models", args.run_name, f"epoch_latest_sam.pth")
        state = {'model': model.float().state_dict(), 'optimizer': optimizer}
        torch.save(state, save_path)
        
        if average_loss < best_loss:
            best_loss = average_loss
            save_path = os.path.join(args.work_dir, "models", args.run_name, f"epoch_best_sam.pth")
            state = {'model': model.float().state_dict(), 'optimizer': optimizer}
            torch.save(state, save_path)
            if args.use_amp:
                model = model.half()
