...
def parse_args():
    ...
    parser.add_argument("--boxes_prompt", action='store_true', default=False, help="use boxes prompt")
    ...
    parser.add_argument("--encoder_adapter", action='store_true', default=False, help="use adapter")
    parser.add_argument("--prompt_path", type=str, default=None, help="fix prompt path")
    parser.add_argument("--rough_prompt", action='store_true', default=False, help="use rough mask as prompt")
    parser.add_argument("--save_pred", type=bool, default=True, help="save result")
    parser.add_argument("--resize_type", type=str, default="nearest", help="resize_type, choose from nearest and linear")
...
def main(args):
    ...
    test_dataset = TestingDataset(data_path=args.data_path, image_size=args.image_size, mode='test', requires_name=True,
                                  ...
                                  resize_type=args.resize_type, rough_prompt=args.rough_prompt)
    ...
    for i, batched_input in enumerate(test_pbar):
        ...
        if args.boxes_prompt:
            ...
            points_show = None
        elif args.rough_prompt:
            save_path = os.path.join(f"{args.work_dir}", args.run_name,
                                     f"rough_iter{args.iter_point}_point{args.point_num}_prompt")
            masks, low_res_masks, iou_predictions = prompt_and_decoder(args, batched_input, model, image_embeddings)
            # todo can not afford iter now
            points_show = None
        else:
            save_path = os.path.join(f"{args.work_dir}", args.run_name, f"iter{args.iter_point}_point{args.point_num}_prompt")
        ...
        if args.save_pred:
            save_masks(masks, save_path, img_name, args.image_size, original_size, pad, batched_input.get("boxes", None), points_show, visual_prompt=False)
        # print(img_name)
    ...
    if args.prompt_path is None:
        with open(os.path.join(args.work_dir,f'{args.image_size}_{args.run_name}_prompt.json'), 'w') as f:
